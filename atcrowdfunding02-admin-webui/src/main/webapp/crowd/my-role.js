...
function fillAuthTree() {
    ...
    // responseJSON可以通过下面那行代码打印到控制台看看是什么
    // console.log(ajaxReturn);
    var authList = ajaxReturn.responseJSON.data;
    // 3.准备对zTree进行设置的]SON对象
    var setting = {
        "data": {
            "simpleData": {
                // 开启简单JSON功能
    ...
    //调用zTree0bj对象的方法，把节点展开
    var zTreeObj = $.fn.zTree.getZTreeObj("authTreeDemo");
    console.log(zTreeObj);
    // 调用zTree0bj对象的方法，把节点展开
    zTreeObj.expandAll(true);
    // 5.查询已分配的Auth的id组成的数组,ajaxReturn已经使用过，就不重复声明变量了
    ajaxReturn = $.ajax({
...
function getPageInfoRemote() {
    ...
    // 判断当前响应状态码是否为200
    if (status != 200) {
        layer.msg("Server-side program call failed! Response code: " + status + " " + ajaxResult.msg());
        return null;
    }
...
function fillTableBody(pageInfo) {
    ...
    if (pageInfo == null || pageInfo == undefined || pageInfo.list == null || pageInfo.list.length == 0) {
        $("#rolePageBody").append("<tr><td colspan='4'>抱歉!没有查询到您搜索的数据!\n</td></tr>>");
        return;
    }
...
function paginationCallBack(pageIndex, jQuery) {
    // 恢复全选框的勾选状态，如果不恢复那么在翻页之前保留的勾选状态会
